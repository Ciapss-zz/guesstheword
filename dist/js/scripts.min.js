...
var results=function($http,$location){var get=function(){return $http.get("https://guesstheword-ed9bc.firebaseio.com/Results.json").then(function(response){return response.data;});};var save=function(results){if(results){return $http.post("https://guesstheword-ed9bc.firebaseio.com/Results.json",results);}else{$location.path('/');}}
...
var WelcomeController=['$scope','$location','getPassObj','results',function($scope,$location,getPassObj,results){results.get($scope.result).then(function(response){$scope.allResults=response;});$scope.startGame=function(name){localStorage.username=name;$location.path('/game');}}];angular.module('myApp.welcome').controller('WelcomeController',WelcomeController);
...
var GameController=['$scope','$http','$location','gameFactory','getPassObj',function($scope,$http,$location,gameFactory,getPassObj){$scope.username=localStorage.username;$scope.start=function(){var currentState={};gameFactory.getWords().then(function(response){currentState.words=response;$scope.currentState=gameFactory.getWord(currentState);document.getElementById('answer-field').focus();document.getElementById('timer-game')['start']();});}
$scope.next=function(){gameFactory.nextWord($scope.currentState);}
$scope.finish=function(){var result={name:localStorage.username,scores:$scope.currentState.scores};localStorage.removeItem("username");getPassObj.passObj(result);$location.path('/result');$scope.$apply();}}];angular.module('myApp.game').controller('GameController',GameController);
...
var ResultController=['$scope','$http','$location','getPassObj','results',function($scope,$http,$location,getPassObj,results){$scope.username
$scope.result=getPassObj.getObj();var getResults=function(){results.get().then(function(response){$scope.allResults=response;});};if($scope.result){results.save($scope.result).then(function(response){getResults();});}else{$location.path('/');}}];angular.module('myApp.result').controller('ResultController',ResultController);